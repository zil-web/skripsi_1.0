...
<body>
<div class="flex h-screen overflow-hidden bg-gray-50" style="--accent: #534AB7;">
    <!-- Sidebar -->
    <aside class="w-56 flex-shrink-0 flex flex-col bg-white border-r">
        ...
        <!-- User identity -->
        @php $kepsek = auth()->guard('kepsek')->user(); @endphp
        <div class="p-4 border-b flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-lg font-semibold text-gray-700">{{ strtoupper(substr($kepsek->nama ?? 'K',0,1)) }}</div>
            <div class="flex-1">
                <div class="font-semibold text-sm">{{ $kepsek->nama ?? 'Kepala' }}</div>
                <div class="text-xs mt-1 inline-flex items-center gap-2">
                    <span class="px-2 py-0.5 rounded text-xs font-medium" style="background:rgba(83,74,183,0.12); color:var(--accent);">Kepala Sekolah</span>
                </div>
            </div>
        </div>
        ...
        <nav class="flex-1 p-2 overflow-y-auto">
            @section('sidebar-menu')
                <a href="{{ route('kepsek.dashboard') }}" class="block px-3 py-2 rounded mb-1" style="{{ request()->routeIs('kepsek.dashboard') ? 'background:var(--accent); color:white;' : '' }}">Dashboard</a>
                <a href="#" class="block px-3 py-2 rounded mb-1">Validasi Transaksi</a>
                <a href="#" class="block px-3 py-2 rounded mb-1">Laporan Keuangan</a>
                <a href="#" class="block px-3 py-2 rounded mb-1">Profil</a>
            @show
        </nav>
        ...
        <header class="bg-white border-b px-6 py-3 flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                    <div class="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center">{{ strtoupper(substr($kepsek->nama ?? 'K',0,1)) }}</div>
                    <div class="text-sm">{{ $kepsek->nama ?? 'Kepala Sekolah' }}</div>
                </div>
            </div>
        </header>
