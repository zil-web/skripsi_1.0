...
@section('sidebar-menu')
    <a href="{{ route('admin.dashboard') }}" class="block px-3 py-2 rounded mb-1 bg-[var(--accent)] text-white">Dashboard</a>
    ...
    <div class="space-y-6" x-data>
            ...
            <div class="rounded-2xl bg-white border border-gray-100 shadow-sm p-5">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-xs text-gray-400">Menunggu validasi kepsek</p>
                    </div>
        ...
        <div class="rounded-2xl bg-white border border-gray-100 shadow-sm overflow-hidden">
            <div class="p-5 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500">10 transaksi terakhir yang tercatat</p>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-100">
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse($transaksi_terbaru as $transaksi)
                            @php
                                $jenis = $toValue($transaksi->jenis);
                                $status = $toValue($transaksi->status);
                            @endphp
                            <tr class="hover:bg-gray-50/70">
                                ...
                                <td class="px-5 py-3 text-gray-700">{{ $transaksi->keterangan }}</td>
                                <td class="px-5 py-3">
                                    <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $jenis === 'pemasukan' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700' }}">
                                        {{ ucfirst($jenis) }}
                                    </span>
                                    <span class="ml-2 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $status === 'approved' ? 'bg-emerald-50 text-emerald-700' : ($status === 'pending' ? 'bg-amber-50 text-amber-700' : 'bg-rose-50 text-rose-700') }}">
                                        {{ ucfirst($status) }}
                                    </span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-10 text-center text-gray-500">Belum ada transaksi.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
